...
def process_image_blocks():
    ...
    with open('data/color_data_rgb_cow.txt', 'w') as file:
    ...
    output_image.save('images/output_color_blocks_cow.jpg')
    print("Processing complete. Check 'color_data_rgb_cow.txt' and 'output_color_blocks_cow.jpg'.")
...
# Function to save tone table to text file in specified format
def save_tone_table(tone_table, filename='data/tone_table.txt'):
    with open(filename, 'w') as file:
        file.write("tone_table = [\n")
        for row in tone_table:
            formatted_row = ', '.join(f"'{tone}'" for tone in row)
            file.write(f"    [{formatted_row}],\n")
        file.write("]\n")
    print(f"Tone table saved to {filename}")

# Process image blocks and update output image
def process_image_blocks():
    num_rows = height // block_size
    num_cols = width // block_size
    tone_table = []  # Dynamic table to store tones

    with open('data/color_data_rgb_cow.txt', 'w') as file:
        for y in range(0, height, block_size):
            row_tones = []  # Temporary list to store tones for the current row
            ...
            for x in range(0, width, block_size):
                ...
                # Store the closest tone in the row list
                row_tones.append(closest_color_info[1])

                # Create a block with the average color for output
                color_block = Image.new('RGB', (box[2] - box[0], box[3] - box[1]), avg_color_tuple)
                output_image.paste(color_block, box)

            tone_table.append(row_tones)  # Add the row to the table

    # Save the output image
    output_image.save('images/output_color_blocks_cow.jpg')
    print("Processing complete. Check 'color_data_rgb_cow.txt' and 'output_color_blocks_cow.jpg'.")

    # Save tone table in specified format
    save_tone_table(tone_table)

    # Print the dynamic tone table
    print("\nTone Table:")
    for row in tone_table:
        print(" | ".join(row))
